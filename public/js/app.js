...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (productList) {
        ...
        const btnAddProduct = document.querySelector('.dashboard-container .btn.add-product');
        const close = document.querySelectorAll('.modal .close');

        btnAddProduct.addEventListener('click', () => {
            document.querySelector('.navbar').style.zIndex = '0';
            createModal.style.display = 'block';
            console.log("se ejecuta el boton agregar producto");
        });
    ...
    if (sections.length > 0) { // Solo si estamos en index.php
        ...
        if (!productList || !catalogContainer) {
            return;
        }
    }
